...
| `internal/logs` | core | docker/kubectl 日志源列表与快照/跟随抓取 |
...
internal/
  ...
  tui/                   model.go(状态+消息) update.go(事件) view.go(布局) styles.go(样式+主题)
                         help.go(帮助浮层) copy.go(OSC52/翻页) follow.go(跟随) picker.go(源选择器)
                         rangefilter.go(Ctrl+T 筛选)
  logs/                  sources.go(ListSources) docker.go(快照/跟随参数)
  config/                config.go
