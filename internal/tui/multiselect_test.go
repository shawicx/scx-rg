...
func TestMultiSelectMarksAndOutputs(t *testing.T) {
	...
	pressKey(t, m, tea.KeyPressMsg{Code: '@', Mod: tea.ModCtrl}) // 标记第 1 条并下移
	...
	if !strings.Contains(m.frame(), "已标记 1") {
		t.Error("状态栏应显示已标记计数")
	}

	pressKey(t, m, tea.KeyPressMsg{Code: '@', Mod: tea.ModCtrl}) // 标记第 2 条（sel 停在末尾）
	if len(m.marked) != 2 {
	...
	pressKey(t, m, tea.KeyPressMsg{Code: '@', Mod: tea.ModCtrl})
	if len(m.marked) != 1 {
		t.Fatalf("重复标记应取消: %v", m.marked)
	}
	pressKey(t, m, tea.KeyPressMsg{Code: tea.KeyUp})
	pressKey(t, m, tea.KeyPressMsg{Code: '@', Mod: tea.ModCtrl}) // 重新标记第 2 条
...
func TestMultiSelectEscProgression(t *testing.T) {
	...
	if !strings.Contains(m.frame(), "已清空标记") {
		t.Error("清标记应有状态栏提示")
	}
}
...
func TestMultiSelectFiltersStaleMarks(t *testing.T) {
	m := newMarkModel(t, "a.txt", "b.txt")
	pressKey(t, m, tea.KeyPressMsg{Code: '@', Mod: tea.ModCtrl}) // 标记 a.txt（假设列表序）
...
func TestMarkedFrameWidthInvariant(t *testing.T) {
	m := newMarkModel(t, "a.txt", "b.txt")
	pressKey(t, m, tea.KeyPressMsg{Code: '@', Mod: tea.ModCtrl})
	pressKey(t, m, tea.KeyPressMsg{Code: tea.KeyF1})
	v := m.frame()
